...
export class AuditLogService {
  ...
  async logRoomCreate(
    ...
    await this.log({
      ...
      resourceId: room.id,
      userId,
      tenantId: tenantId || room.tenantId,
      ipAddress,
  ...
  async logGuestCreate(
    ...
    await this.log({
      ...
      resourceId: guest.id,
      userId,
      tenantId: tenantId || guest.tenantId,
      ipAddress,
  ...
  async logMaintenanceCreate(
    ...
    await this.log({
      ...
      resource: AuditResource.MAINTENANCE_TASK,
  ...
  async logPropertyUpdate(
    ...
    await this.log({
      ...
      resource: AuditResource.PROPERTY,
  ...
  private async resolveUserNames(
    ...
    userMap: Map<string, { name: string; email: string; role: string }>;
    adminMap: Map<string, { name: string; email: string; role: string }>;
  }> {
  ...
  async getLogs(query: AuditLogQueryDto, tenantId?: string) {
    ...
    const data = rawData.map((log) => {
      const info = log.userId
        ? userMap.get(log.userId)
        : log.adminId
          ? adminMap.get(log.adminId)
          : undefined;
    ...
    try {
      ...
      return {
        ...
        byCategory: Array.from(categoryMap.entries())
          ...
          .sort((a, b) => b.count - a.count),
        byResourceType: Array.from(new Map<string, number>().entries()).map(
