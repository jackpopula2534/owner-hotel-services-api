...
export class HrAttendanceService {
  ...
  async checkIn(dto: CheckInDto, tenantId: string) {
    ...
    const status =
      checkIn.getUTCHours() >= lateThresholdHour ? 'late' : 'present';
  ...
  async getSummary(query: Record<string, string>, tenantId: string) {
    ...
    const [present, late, absent, onLeave] = await Promise.all([
      (this.prisma as any).hrAttendance.count({ where: { tenantId, status: 'present',  ...(Object.keys(dateWhere).length && { date: dateWhere }) } }),
      (this.prisma as any).hrAttendance.count({ where: { tenantId, status: 'late',     ...(Object.keys(dateWhere).length && { date: dateWhere }) } }),
      (this.prisma as any).hrAttendance.count({ where: { tenantId, status: 'absent',   ...(Object.keys(dateWhere).length && { date: dateWhere }) } }),
      (this.prisma as any).hrAttendance.count({ where: { tenantId, status: 'on_leave', ...(Object.keys(dateWhere).length && { date: dateWhere }) } }),
    ]);
